...
针对交换流程在**重复刷新与重放操作后的记录稳定性**的可重复执行回归套件（`scripts/persistence-regression/`）。规则：

- 每个阶段结束后都从持久化层重新加载，并校验 localStorage 镜像与 IndexedDB 双写一致；
- 任一阶段发现同一交换/履约单/确认记录重复或履约码重复即失败，失败信息指出首次重复发生在哪个阶段；
- 每个阶段的操作（同意、确认、收口、释放）都会重放，重放后不得产生重复记录；
- 覆盖场景：单交换全生命周期、并发同意与共享物品归属冲突、收口失败回滚与受阻恢复、全量重放与连续刷新稳定性；
- 最终归属（哪条交换完成）与物品结果（哪张物品已交换）必须与预期一致。
